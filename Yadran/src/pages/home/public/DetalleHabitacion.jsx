...
import { obtenerEstado } from "../../../config/utils/estadosConfig";
...
const DetalleHabitacion = () => {
    ...
    useEffect(() => {
        const cargarHabitacion = async () => {
            try {
                setCargando(true);
                const respuesta = await fetch(`http://localhost:3001/habitaciones/${id}`);
    ...
    const infoEstado = obtenerEstado(habitacion.estado) || {
        variante: 'secondary',
        etiqueta: habitacion.estado,
        icono: null
    };
                    ...
                    <Row>
                        <Col lg={8}>
                            <div className="d-flex justify-content-between align-items-center mb-4 flex-wrap">
                                <div>
                                    <h1 className="mb-2">{habitacion.nombre || `Habitación ${habitacion.id.split('-')[1] || ''}`}</h1>
                                    <div className="d-flex align-items-center flex-wrap">
                                        ...
                                        >
                                            {infoEstado.icono && (
                                                <span className="me-1">{infoEstado.icono}</span>
                                            )}
                                            {infoEstado.etiqueta}
                                        </Badge>
                                ...
                                <Row className="caracteristicas-basicas mt-4">
                                    ...
                                    <Col md={4} className="mb-3">
                                        <Card className="h-100 border-0 shadow-sm p-3 text-center">
                                            ...
                                            <h5>Precio</h5>
                                            <p className="mb-0">${habitacion.precio}/noche</p>
                                        </Card>
                                    </Col>
                                </Row>
                            </div>
                            ...
                            <Card className="reserva-card sticky-lg-top border-0 shadow-sm" style={{ top: "2rem" }}>
                                ...
                                <Card.Body>
                                    <div className="precio-resumen text-center mb-4">
                                        <span className="precio-valor-grande">${habitacion.precio}</span>
                                        ...
                                        {fechaEntrada && fechaSalida && (
                                            <div className="resumen-reserva mb-4">
                                                ...
                                                <div className="d-flex justify-content-between fw-bold pt-2 border-top">
                                                    <span>Total</span>
                                                    <span>
                                                        ${habitacion.precio * Math.max(1, Math.ceil((new Date(fechaSalida) - new Date(fechaEntrada)) / (1000 * 60 * 60 * 24)))}
                                                    </span>
                                                </div>
                                            </div>
                                        )}
